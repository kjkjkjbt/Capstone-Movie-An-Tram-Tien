...
const ListGhe = ({ maLichChieu }) => {
  ...
  const renderListGhe = () => {
    return listGhe.map((ghe) => {
      let style = 'bg-blue-500';
      let isDisabled = false;

      const index = listGheDangDat.findIndex((gheDangDat) => ghe.maGhe === gheDangDat.maGhe);
      if (index !== -1) {
        style = 'bg-red-500';
      }
      ...
      return (
        <button
          key={ghe.maGhe}
          disabled={isDisabled}
          onClick={() => dispatch(addGheAction(ghe))}
          className={`border rounded p-2 text-white ${style} ${isDisabled ? 'cursor-not-allowed' : ''}`}
        >
          {ghe.tenGhe}
        </button>
      );
